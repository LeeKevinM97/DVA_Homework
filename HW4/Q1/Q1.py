...
def author():                                                                                             
    return "klee876" # replace gburdell3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903762294 # replace with your GT ID number      
...
class PageRank:
    ...
    def calculate_node_degree(self):
        ...
        # Given source isn't already in the degree dictionary, add it with a value of 1, since first edge
            if source not in self.node_degree.keys():
                self.node_degree[source] = 1
        # If source already in degree dictionary, add +1 to value
            elif source in self.node_degree.keys():
                self.node_degree[source] += 1

        # If source or target is larger that max_node_id, update it
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        ### Implement your code here
        #############################################
            # Next Page Rank Value for Node J = ( 1 - d ) * Prob Factor + d * Summation of ( Page Rank Value for Node / Out Degrees ) for all nodes pointing to Node J
                new_pr_values[target] += pr_values[source] / self.node_degree[source]
            for node, value in enumerate(pr_values):
                pr_values[node] = ( 1 - damping_factor ) * node_weights[node] + damping_factor * new_pr_values[node]
        #############################################
